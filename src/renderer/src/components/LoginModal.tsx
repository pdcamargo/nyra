...
export default function LoginModal(): React.JSX.Element | null {
  ...
  useEffect(() => {
    ...
    exitUnsubRef.current = window.api.login.onExit(({ exitCode }) => {
      ...
      if (success) {
        window.dispatchEvent(new CustomEvent('nyra:login-success'))
  ...
  return (
    <Modal onClose={handleCancel} title="Sign in to Claude" className="max-w-2xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-3 border-b border-border">
          <div>
            <h2 className="text-[14px] font-semibold text-foreground">Sign in to Claude</h2>
