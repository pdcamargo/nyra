...
const STATUS_BADGE: Record<FileStatus | 'reverted' | 'deleted', { label: string; className: string }> = {
  created: { label: 'new', className: 'bg-success/20 text-success' },
  modified: { label: 'modified', className: 'bg-info/20 text-info' },
  read: { label: 'read', className: 'bg-accent text-muted-foreground' },
  reverted: { label: 'reverted', className: 'bg-warning/20 text-warning' },
  deleted: { label: 'deleted', className: 'bg-danger/20 text-danger/60' }
}
  ...
  return (
    <div>
      <div className="flex items-center justify-between px-1 mb-2">
        <p className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/70">Files</p>
        {parts.length > 0 && (
          <span className="text-[10px] text-muted-foreground font-mono">{parts.join(' · ')}</span>
        )}
      </div>
...
const FileChangelogRow = memo(function FileChangelogRow({
  ...
  return (
    <div className="rounded-md border border-transparent hover:border-border/55 transition-colors">
      <div
        className={`flex items-center gap-2 px-2 py-1.5 ${expandable ? 'cursor-pointer' : ''}`}
        onClick={handleExpand}
      >
        {expandable && (
          <span className={`text-[10px] text-muted-foreground/70 transition-transform ${expanded ? 'rotate-90' : ''}`}>
            ▶
          </span>
        )}
        <div className="min-w-0 flex-1">
          <span className={`text-[11px] truncate block ${alreadyGone ? 'text-muted-foreground/70 line-through' : 'text-foreground/80'}`}>
            {fileName}
          </span>
          {dirPath && (
            <span className="text-[10px] text-muted-foreground/70 truncate block">{dirPath}</span>
          )}
        </div>
        <div className="flex items-center gap-1.5 shrink-0">
          {entry.editCount > 1 && (
            <span className="text-[10px] text-muted-foreground/70 font-mono">{entry.editCount} edits</span>
          )}
      ...
      {expanded && expandable && (
        <div className="px-2 pb-2">
          {loading && (
            <div className="flex items-center justify-center py-4 text-muted-foreground/70 text-xs">
              Loading...
            </div>
          )}
          {alreadyGone && (
            <div className="text-[11px] text-muted-foreground/70 py-2 px-1">File no longer exists on disk</div>
          )}
              ...
              <button
                ...
                className={`text-[10px] px-2 py-0.5 rounded transition-colors ${
                  confirming
                    ...
                    : 'bg-accent/50 text-muted-foreground hover:text-foreground/80 hover:bg-accent'
                }`}
              >
