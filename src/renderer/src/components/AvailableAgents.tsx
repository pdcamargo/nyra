  ...
  return (
    <div>
      <div className="flex items-center justify-between px-1 mb-2">
        ...
        <span className="text-[10px] text-muted-foreground font-mono">{agents.length}</span>
      </div>
...
function AgentRow({
  ...
  return (
    <div className="group flex items-start gap-2 rounded-md px-2 py-1.5 hover:bg-accent/50 transition-colors">
      <AgentIcon icon={icon} />
      <button
        onClick={onDispatch}
      ...
      >
        <div className="flex items-center gap-1.5">
          ...
          {agent.scope === 'global' && (
            <span className="text-[8px] uppercase tracking-wider text-muted-foreground/70 shrink-0">
              global
            </span>
          )}
        </div>
      ...
      <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
        <button
          onClick={onEdit}
          className="p-1 rounded-sm hover:bg-accent text-muted-foreground/70 hover:text-foreground/80"
        ...
        <button
          onClick={onDispatch}
          className="p-1 rounded-sm hover:bg-accent text-muted-foreground/70 hover:text-info"
...
function AgentIcon({ icon }: { icon: IconDef }): React.JSX.Element {
  return (
    <span
      className={`shrink-0 mt-0.5 ${icon.tone}`}
