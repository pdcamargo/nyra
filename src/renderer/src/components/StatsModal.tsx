...
function StatRow({ label, value, color }: { label: string; value: string; color?: string }): React.JSX.Element {
  return (
    <div className="flex justify-between items-center py-1.5">
      ...
      <span className={`text-[11px] font-mono ${color ?? 'text-foreground/80'}`}>{value}</span>
    </div>
  )
}
...
export default function StatsModal({ onClose }: { onClose: () => void }): React.JSX.Element {
  ...
  return (
    <Modal onClose={onClose} title="Usage and cost" className="max-w-sm p-5">
        {/* Header */}
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-semibold text-foreground">Session Stats</h2>
          <button
            onClick={onClose}
        ...
        <div className="rounded-lg border border-border/55 bg-muted/40 p-3 mb-3">
          ...
          {session?.claudeSessionId && (
            <StatRow label="Session ID" value={session.claudeSessionId.slice(0, 12)} color="text-muted-foreground" />
          )}
        </div>
          ...
          <div className="h-1.5 w-full rounded-full bg-accent mb-2">
            <div
              className={`h-1.5 rounded-full transition-all duration-500 ${
                contextPct > 90 ? 'bg-danger/70' : contextPct > 70 ? 'bg-warning/60' : 'bg-info/60'
              }`}
