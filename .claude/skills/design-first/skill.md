...
2. **Set up Pencil**
   ...
   - Call `get_style_guide_tags()` then `get_style_guide(tags)` with tags matching nyra's aesthetic: `dark-mode`, `devtools`, `minimal`, `monospace`, `webapp`, `developer`, `functional`

3. **Build the mockup**
   - Create frames representing the UI states (e.g., empty state, populated state, active state)
   - Use nyra's visual language: dark backgrounds (#111111, #0A0A0A), subtle borders (#1a1a1a, #ffffff0f), JetBrains Mono for UI text, muted white text colors
